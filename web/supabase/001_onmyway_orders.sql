...
-- The Expo app works before this runs (it drops columns the table doesn't have),
-- but without it the courier's name/UPI, fare, PIN expiry, ratings and reports
-- are not shared between phones.
...
alter table public.orders
  ...
  add column if not exists delivery_fee     integer,       -- ₹40 regular / ₹55 large, quoted when placed
